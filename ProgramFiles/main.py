...
for address in addresses:
    ...
    HostpointClient.upload_to_Hostpoint(config['HOSTPOINT']['hostname'], config['HOSTPOINT']['username'],
                                        config['HOSTPOINT']['password'], plc_dest, new_files)
...
# log into the email
sender = EMailSender.setup_email_sender(config['EMAIL']['smtp_server'], config['EMAIL']['smtp_port'],
                                        config['EMAIL']['username'], config['EMAIL']['password'])

# send the emails
if len(failed_connections) > 0:
    sender.send_email(config['EMAIL']['username'], zip(names, emails), 'email_templates/ConnectionError.temp',
                      "SPS Connection Error", failed_connections)

if len(full_plcs) > 0:
    sender.send_email(config['EMAIL']['username'], zip(names, emails), 'email_templates/SPS_FullWarning.temp',
                      "SPSs Are Full", full_plcs)

if len(days_rem) > 0:
    sender.send_email_almost_full(config['EMAIL']['username'], zip(names, emails),
                                  'email_templates/DaysLeftWarning.temp', "SPSs Are Almost Full", days_rem,
                                  config['EMAIL']['warning_days'])
